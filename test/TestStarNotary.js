...
it('lets 2 users exchange stars', async() => {
  ...
  let tokenId2 = 8;
    ...
    await instance.createStar('StarOne', tokenId1, {from: accounts[0]});
    await instance.createStar('StarTwo', tokenId2, {from: accounts[1]});
    assert.equal(await instance.ownerOf(tokenId1), accounts[0]);
...
it('lets a user transfer a star', async() => {
  let instance = await StarNotary.deployed();
    // 1. create a Star with different tokenId
  let tokenId = 9;
  await instance.createStar('Yeeha', tokenId, {from: accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
  let instance = await StarNotary.deployed();
    // 1. create a Star with different tokenId
  let tokenId = 10;
  await instance.createStar('Hehe', tokenId, {from: accounts[0]});
    // 2. Call your method lookUptokenIdToStarInfo
    // 3. Verify if you Star name is the same
    assert.equal(await instance.lookUptokenIdToStarInfo(tokenId), 'Hehe');
});
